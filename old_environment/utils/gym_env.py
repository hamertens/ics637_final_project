...
class SkidSteerEnv(gym.Env):
    ...
    def _calculate_reward(self):
        ...
        scaled_kappa = linear_interpolation(kappa, -4.4087, 6.0996)
        scaled_z = linear_interpolation(z, -0.0093, 3.409)
        ...
        if np.allclose(self.state, skid_steer_dynamics_with_obstacles(self.state, 0, 0, self.df, self.obstacle_tree)):
            reward -= 10  # Large penalty for collisions (staying in the same state)
        ...
        if distance_to_goal < 0.1:  # Threshold for reaching the goal
            reward += 1000000 / self.steps_taken  # Larger bonus for quicker goal achievement
